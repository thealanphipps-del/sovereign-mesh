...
def run_via_ssh(host, command):
    ...
    """
    import subprocess
    log(f"[SSH-FALLBACK] Routing execution via native SSH shell interface to {BOLD}{host}{RESET}...", color=GOLD)
...
def print_processes_table(processes, plane_source):
    ...
    print(f"{'PID':<7} | {'PPID':<7} | {'USER':<12} | {'STAT':<5} | {'%CPU':<5} | {'%MEM':<5} | {'NAME':<20} | {'COMMAND'}")
    print("-" * 110)
    ...
    for p in sorted_procs[:50]: # Top 50 processes
        ...
        print(f"{p.pid:<7} | {p.ppid:<7} | {p.username:<12} | {p.status:<5} | {p.cpu_percent:<5.1f} | {p.memory_percent:<5.1f} | {p.name:<20} | {cmd_truncated}")
    print("-" * 110 + "\n")
...
def print_ports_table(bindings, plane_source):
    ...
    print(f"{'PROTO':<5} | {'LOCAL ADDRESS':<22} | {'PORT':<6} | {'PEER ADDRESS':<22} | {'PORT':<6} | {'STATE':<12} | {'PID':<6} | {'PROCESS NAME'}")
    print("-" * 110)
    ...
    for b in sorted_binds:
        ...
        print(f"{b.protocol:<5} | {local_addr:<22} | {b.local_port:<6} | {peer_addr_trunc:<22} | {b.remote_port:<6} | {b.state:<12} | {b.pid:<6} | {b.process_name}")
    print("-" * 110 + "\n")
...
def send_over_memory_bus(host, port, data_bytes, offset=0, page_size=4096):
    ...
    try:
        ...
        log(f"Paging agent mind state ({file_size} B) into {total_pages} pages...", color=BLUE)
        ...
        for page_idx in range(total_pages):
            ...
            header = struct.pack(HEADER_FORMAT, MAGIC, page_idx, page_offset, actual_len, crc)
            sock.sendall(header + chunk_data)
            
        sock.close()
        log(f"High-Speed RAM state copy complete! {file_size} bytes synced in RAM.", color=GREEN)
        return True
    except Exception as e:
...
def run_suite(host, port, args):
    target = f"{host}:{port}"
    log(f"Establishing insecure connection channel to {BOLD}{target}{RESET}...", color=BLUE)
    ...
    try:
        t0 = time.time()
        ping_res = stub.Ping(sync_pb2.PingRequest(client_id=client_id, timestamp=int(t0 * 1000)))
        t1 = time.time()
        latency = (t1 - t0) * 1000
        log(f"Ping Ack received! Server: {BOLD}{ping_res.server_id}{RESET} | Status: {ping_res.status} | Latency: {latency:.2f} ms", color=GREEN)
        grpc_online = True
        server_id = ping_res.server_id
    except Exception as e:
        log(f"Ping failed: {e}", color=RED)
        log(f"{BOLD}{RED}[WARNING] gRPC control plane is UNRESPONSIVE at {target}!{RESET}", color=RED)
        log(f"{BOLD}{GOLD}[FALLBACK] Failover: Initiating SSH Remote Execution plane...{RESET}", color=GOLD)

    # A. ADD USER
    if args.adduser:
        if grpc_online:
            log(f"Executing RPC: CreateUser for {BOLD}{args.adduser}{RESET}...", color=GOLD)
            try:
        ...
        if not grpc_online:
            ...
            if code == 0:
                log(f"Success: Created user {args.adduser} via SSH fallback.", color=GREEN)
            else:
                log(f"Failed to create user via SSH fallback (code {code}): {stderr.strip()}", color=RED)
        return

    # B. CHANGE PASSWORD
    elif args.chpasswd:
        if not args.password:
            log("Error: --password <new_password> is required to change password.", color=RED)
            return
            
        if grpc_online:
            log(f"Executing RPC: ChangePassword for {BOLD}{args.chpasswd}{RESET}...", color=GOLD)
            try:
                req = sync_pb2.ChangePasswordRequest(
                    username=args.chpasswd,
        ...
        if not grpc_online:
            ...
            code, stdout, stderr = run_via_ssh(host, cmd_str)
            if code == 0:
                log(f"Success: Changed password for {args.chpasswd} via SSH fallback.", color=GREEN)
            else:
                log(f"Failed to change password via SSH (code {code}): {stderr.strip()}", color=RED)
        return
    ...
    # C. MANAGE GROUP
        ...
        if action_enum is None:
            log(f"Error: Invalid action '{args.action}'. Must be add, remove, or create.", color=RED)
            return
            
        if action_enum in [sync_pb2.ManageGroupRequest.ADD_TO_GROUP, sync_pb2.ManageGroupRequest.REMOVE_FROM_GROUP] and not args.username:
            log("Error: --username <username> is required when adding or removing from a group.", color=RED)
            return
            
        if grpc_online:
            log(f"Executing RPC: ManageGroup for group {BOLD}{args.manage_group}{RESET}...", color=GOLD)
            try:
        ...
        if not grpc_online:
            ...
            code, stdout, stderr = run_via_ssh(host, cmd_str)
            if code == 0:
                log(f"Success: Group management action '{args.action}' completed via SSH fallback.", color=GREEN)
            else:
                log(f"Failed to perform group management via SSH (code {code}): {stderr.strip()}", color=RED)
        return
    ...
    # D. LIST USERS
        ...
        if grpc_online:
            ...
            try:
                ...
                print(f"{'USERNAME':<16} | {'UID':<6} | {'GID':<6} | {'PRIMARY GROUP':<16} | {'SHELL':<16} | {'SECONDARY GROUPS'}")
                print("-" * 90)
                for u in res.users:
                    sg_str = ", ".join(u.groups) or "None"
                    print(f"{u.username:<16} | {u.uid:<6} | {u.gid:<6} | {u.primary_group:<16} | {u.shell:<16} | {sg_str}")
                print("-" * 90 + "\n")
            except Exception as e:
        ...
        if not grpc_online:
            ...
            if code == 0:
                try:
                    ...
                    log(f"Failed to parse user directory JSON from SSH output: {ex}", color=RED)
                    print(f"Raw Output:\n{stdout}")
            else:
                log(f"Failed to fetch user directory via SSH fallback (code {code}): {stderr.strip()}", color=RED)
        return
    ...
    # E. SYNC USERS
        ...
        try:
            ...
            for p in pwd.getpwall():
                if p.pw_uid >= 1000 and p.pw_name != "nobody":
                    ...
                    local_users.append(sync_pb2.UserInfo(
                        username=p.pw_name,
                        uid=p.pw_uid,
                        gid=p.pw_gid,
        ...
        if grpc_online:
            log(f"Executing RPC: SyncUsers to remote target {BOLD}{target}{RESET}...", color=GOLD)
            try:
        ...
        if not grpc_online:
            log(f"Re-routing synchronization over SSH for {len(local_users)} users...", color=GOLD)
            details = []
            success_count = 0
            for u in local_users:
                ...
                if code != 0:
                    ...
                    if c_code == 0:
                        details.append(f"SSH Sync: Created user {u.username} (UID={u.uid})")
                        success_count += 1
                    else:
                        details.append(f"SSH Sync Error: Failed to create user {u.username}: {c_stderr.strip()}")
                else:
                    ...
                    if q_code == 0:
                        ...
                        if needs_update:
                            ...
                            m_code, m_stdout, m_stderr = run_via_ssh(host, " ".join(mod_cmd))
                            if m_code == 0:
                                details.append(f"SSH Sync: Aligned properties for user {u.username}")
                                if u.uid != existing_uid:
                                    run_via_ssh(host, f"sudo chown -R {u.username}:{u.gid} {u.home_dir}")
                            else:
                                details.append(f"SSH Sync Error: Failed to align {u.username}: {m_stderr.strip()}")
                        success_count += 1
                    else:
                        details.append(f"SSH Sync Error: Failed to query existing user {u.username}")
            
            log(f"SSH User Sync complete. Successful: {success_count}/{len(local_users)}", color=GREEN)
            for det in details:
    ...
    # F. PROCESS DIRECTORY LISTING
        ...
        if grpc_online:
            log(f"Executing RPC: GetProcessDirectory on {BOLD}{target}{RESET}...", color=GOLD)
            try:
                req = sync_pb2.ProcessDirectoryRequest()
                res = stub.GetProcessDirectory(req)
                print_processes_table(res.processes, f"gRPC CONTROL PLANE ({server_id})")
            except Exception as e:
        ...
        if not grpc_online:
            ...
            if code == 0:
                ...
                log(f"SSH fallback failed with code {code}: {stderr.strip()}", color=RED)
        return

    # G. PORT BINDINGS LISTING
    elif args.ports:
        if grpc_online:
            log(f"Executing RPC: GetPortBindings on {BOLD}{target}{RESET}...", color=GOLD)
            try:
        ...
        if not grpc_online:
            ...
            if code_tcp == 0 or code_udp == 0:
                ...
                log(f"SSH fallback failed. TCP code {code_tcp}: {stderr_tcp} | UDP code {code_udp}: {stderr_udp}", color=RED)
        return

    # H. REMOTE COMMAND EXECUTION
    elif args.exec:
        if grpc_online:
            log(f"Executing RPC: RemoteExecute for command '{BOLD}{args.exec}{RESET}' on {BOLD}{target}{RESET}...", color=GOLD)
            try:
    ...
    # I. VIRTUAL AGENT TRAVEL (TELEPORTATION)
        ...
        if grpc_online:
            log(f"Executing RPC: TeleportAgent to materialize agent on {target}...", color=GOLD)
            try:
                ...
                if res.success:
                    ...
                    log(f"Virtual Travel Materialization rejected by remote node: {res.message}", color=RED)
                    grpc_online = False
            except Exception as e:
        ...
        if not grpc_online:
            log("[WARNING] gRPC plane unresponsive! Triggering SSH materialization...", color=GOLD)
    ...
    # J. AGENT PEDIGREE TRACING
        ...
        if grpc_online:
            log(f"Executing RPC: TracePedigree for '{BOLD}{target_agent}{RESET}' on {BOLD}{target}{RESET}...", color=GOLD)
            try:
                ...
                for idx, node in enumerate(res.pedigree_path):
                    ...
                    print(f"{indent}{connector}{BOLD}{node.agent_id}{RESET} ({node.name})")
                    print(f"{indent}    Layer {node.layer_level} | Specialty: {GOLD}{node.specialty}{RESET} -> Subspecialty: {CYAN}{node.subspecialty}{RESET}")
                print("-" * 70)
        ...
        if not grpc_online:
            log("[WARNING] gRPC plane unresponsive! Querying SQLite Pedigree DB over SSH...", color=GOLD)
    ...
    # K. SWARM MUTATION PROPOSAL
        ...
        if grpc_online:
            log(f"Executing RPC: ProposeSwarmMutation for '{BOLD}{target_key}{RESET}' -> '{BOLD}{proposed_value}{RESET}' on {BOLD}{target}{RESET}...", color=GOLD)
            try:
                ...
                if res.block_index > 0:
                    ...
                    print(f"{BOLD}Block Cryptographic Hash:{RESET} {GOLD}{res.block_hash}{RESET}")
                
                print(f"\n{BOLD}Representative Vote Mapping:{RESET}")
                for vote in res.votes:
                    marker = f"{GREEN}✔ AGREE{RESET}" if vote.vote_agree else f"{RED}✘ DISAGREE{RESET}"
                    print(f"  [{marker}] {BOLD}{vote.agent_id}{RESET} - Rationale: {vote.rationale}")
        ...
        if not grpc_online:
            log("[WARNING] gRPC plane unresponsive! Cannot safely evaluate consensus over SSH fallback (4/5 consensus requires gRPC control plane orchestration). Try starting the gRPC service.", color=RED)
        return

    # L. SWARM LEDGER AUDIT
    elif args.ledger:
        if grpc_online:
            log(f"Executing RPC: QuerySwarmLedger on {BOLD}{target}{RESET}...", color=GOLD)
            try:
                req = sync_pb2.LedgerQueryRequest()
                res = stub.QuerySwarmLedger(req)
                
                print(f"\n{BOLD}{GREEN}--- IMMUTABLE LEDGER BLOCKCHAIN AUDIT ---{RESET}")
                print(f"{BOLD}Validation Status:{RESET} {GOLD}{res.chain_validation_status}{RESET}")
                print("=" * 100)
                
                for block in res.blocks:
                    print(f"{BOLD}Block #{block.block_index}{RESET} | Proposer: {BOLD}{block.agent_id}{RESET} | Timestamp: {block.timestamp}")
                    print(f"  {BOLD}Prev Hash:{RESET} {block.previous_hash[:16]}... | {BOLD}Block Hash:{RESET} {GOLD}{block.block_hash[:16]}...{RESET}")
                    
                    payload = json.loads(block.mutation_payload)
                    print(f"  {BOLD}Payload:  {RESET} {json.dumps(payload, indent=2).replace(chr(10), chr(10)+'  ')}")
                    ...
                    if block.minority_report and block.minority_report != "None":
                        ...
                        for line in lines[:8]:
                            print(f"    {line}")
                        if len(lines) > 8:
                            print("    [Truncated... Check pedigree database for complete logs]")
                    print("-" * 100)
        ...
        if not grpc_online:
            log("[WARNING] gRPC plane unresponsive! Reading SQLite Ledger directly over SSH...", color=GOLD)
    ...
    # M. JETWEB TIME MACHINE OVERRIDE
        ...
        if not args.rewrite:
            log("Error: Direct timeline mutation rewrite parameter '--rewrite <key>=<value>' is required to execute override.", color=RED)
            return
        ...
        if grpc_online:
            log(f"Executing RPC: TimeTravelOverride for Destination Block #{target_idx}...", color=CYAN)
            try:
                ...
                print(f"{BOLD}Overriding Target Decision at Block Index:{RESET} #{target_idx}")
                print(f"{BOLD}Timeline State Refactor Action:{RESET} Set {target_key} = {proposed_value}")
                print(f"{BOLD}Status:{RESET} {'SUCCESS' if res.success else 'FAILED'}")
                print(f"{BOLD}Audit Verification Status:{RESET} {GOLD}{res.new_chain_validation_status}{RESET}")
        ...
        if not grpc_online:
            log("[WARNING] gRPC plane unresponsive! Cannot safely compute blockchain fork or re-mine cascade hashes over SSH fallback.", color=RED)
        return
    ...
    # N. SWARM FORENSIC DECISION ACCOUNTING
        ...
        if grpc_online:
            log(f"Executing RPC: ForensicAudit (Block Filter: {target_idx or 'ALL'})...", color=GOLD)
            try:
                ...
                print(f"{BOLD}Active relational decision logs and ancestry state traces:{RESET}")
                print("=" * 110)
                ...
                for node in res.timeline_nodes:
                    ...
                    print(f"{BOLD}Block Index:{RESET} #{node.block_index} | {BOLD}Timestamp:{RESET} {node.timestamp} | {BOLD}Agent:{RESET} {node.agent_id}")
                    print(f"  {BOLD}Hash:{RESET} {GOLD}{node.block_hash}{RESET}")
                    print(f"  {BOLD}Decision Event Payload:{RESET}")
                    print(f"    Action: {payload.get('action')} | Key: {BOLD}{payload.get('key')}{RESET} | Value: {BOLD}{payload.get('value')}{RESET}")
                    print(f"    Reason: {payload.get('reason')}")
                    
                    print(f"  {BOLD}Consensus Vote Trace:{RESET}")
                    for vote in votes:
                        vote_marker = f"{GREEN}✔ AGREE{RESET}" if vote.get('vote_agree') else f"{RED}✘ DISAGREE{RESET}"
                        print(f"    - {vote_marker} {BOLD}{vote.get('agent_id')}{RESET}: '{vote.get('rationale')}'")
                        
                    if node.minority_report and node.minority_report != "None":
                        print(f"  {BOLD}Minority Analysis & Dissent Cause/Effect Matrix:{RESET}")
        ...
        if not grpc_online:
            log("[WARNING] gRPC plane unresponsive! Reading database directly over SSH fallback plane...", color=GOLD)
    ...
    if args.prompt:
        log(f"Executing RPC: StreamInference with prompt: \"{args.prompt}\"...", color=GOLD)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--port", type=int, default=1111, help="Target gRPC server port")
    parser.add_argument("--prompt", help="Optional prompt to execute remote streaming inference test")
    
    # Process & Port Monitoring options
    parser.add_argument("--ps", action="store_true", help="List running processes on the remote node")
    parser.add_argument("--ports", action="store_true", help="List active port bindings on the remote node")
    parser.add_argument("--exec", help="Execute a shell command remotely on the target host (with SSH failover)")
    
    # User Maintenance options
    parser.add_argument("--adduser", help="Add a new user to the remote node")
    parser.add_argument("--password", help="Password for the user (used with --adduser or --chpasswd)")
    parser.add_argument("--uid", type=int, default=0, help="Explicit UID (used with --adduser)")
    parser.add_argument("--group", help="Primary group or GID (used with --adduser)")
    parser.add_argument("--shell", default="/bin/bash", help="Shell for the new user (default: /bin/bash)")
    parser.add_argument("--no-create-home", action="store_true", help="Do not create home directory for new user")
    
    parser.add_argument("--chpasswd", help="Change password for a user on the remote node")
    
    parser.add_argument("--manage-group", help="Manage a group on the remote node")
    parser.add_argument("--action", choices=["create", "add", "remove"], help="Action for group management")
    parser.add_argument("--username", help="Username to add to/remove from the group")
    parser.add_argument("--gid", type=int, default=0, help="Explicit GID for creating group")
    
    parser.add_argument("--list-users", action="store_true", help="List all users on the remote node")
    parser.add_argument("--sync-users", action="store_true", help="Synchronize local users (UID >= 1000) to the remote node")
    
    # Virtual Agent Travel options
    parser.add_argument("--travel", help="Virtually travel and migrate the agent to the remote server, executing a resume command upon arrival")
    parser.add_argument("--bus-port", type=int, default=11111, help="Target memory bus port for travel state transmission")
    
    # Agent Pedigree option
    parser.add_argument("--pedigree", nargs="?", const="", help="Trace agent pedigree back to Agent 0 and view 7-layer subspecialty cooperation network")
    
    # Swarm Ticketing and Relational Ledger options
    parser.add_argument("--propose", help="Propose a relational memory/knowledge base mutation in key=value format")
    parser.add_argument("--reason", help="Detailed rationale for the proposed mutation (influences consensus voting)")
    parser.add_argument("--ledger", action="store_true", help="Audit the immutable multi-dimensional blockchain ledger")
    
    # Jetweb Time Machine options
    parser.add_argument("--timemachine", help="Target block index to override retroactively via Jetweb Time Machine")
    parser.add_argument("--rewrite", help="Override payload key=value format to apply retroactively")
    parser.add_argument("--forensics", nargs="?", const="0", help="Perform retroactive forensic accounting of agent decisions")
