...
def initialize_shared_memory():
    ...
    log(f"Initializing 16MB shared memory swap at: {BOLD}{PAGE_TABLE_PATH}{RESET}...", color=BLUE)
    
    # Ensure path directory exists (fallback to /tmp if run outside WSL)
    dir_name = os.path.dirname(PAGE_TABLE_PATH)
    if not os.path.exists(dir_name):
        log(f"Directory {dir_name} not found! Falling back to /tmp/sovereign_page_table", color=RED)
    ...
    try:
        ...
        log(f"Shared memory successfully mapped! Base Address: {BOLD}0x{mapped_memory.tell():08X}{RESET}", color=GREEN)
...
def handle_client_connection(client_socket, client_address, mapped_memory):
    log(f"High-Speed Channel opened from {BOLD}{client_address[0]}:{client_address[1]}{RESET}", color=GOLD)
    ...
    try:
        while True:
            ...
            magic, page_index, offset, page_size, checksum = struct.unpack(HEADER_FORMAT, header_bytes)
            
            if magic != MAGIC:
                log(f"Invalid magic number {hex(magic)}! Terminating channel.", color=RED)
                break
            ...
            if len(payload_bytes) < page_size:
                log(f"Incomplete page received! Expected {page_size} bytes, got {len(payload_bytes)}", color=RED)
                break

            # 3. Verify Checksum
            calculated_crc = binascii.crc32(payload_bytes) & 0xffffffff
            if calculated_crc != checksum:
                log(f"Checksum mismatch on Page {page_index}! Dropping page sync.", color=RED)
                continue

            # 4. Direct Zero-Copy Fast Write to RAM Backed mmap
            if offset + page_size > TOTAL_BUS_SIZE:
                log(f"Memory bounds violation! Offset {offset} + Page {page_size} exceeds {TOTAL_BUS_SIZE}", color=RED)
                break
            ...
            if pages_copied % 100 == 0 or page_size > 65536: # Log occasionally or for large paging
                log(f"Syncing Page {page_index:04d} -> Mapped Offset 0x{offset:06X} | Size: {page_size}B | Latency: {write_time_us:.2f}μs", color=CYAN)

        elapsed = time.time() - start_time
        if elapsed > 0:
            speed = (total_bytes_received / (1024 * 1024)) / elapsed
            log(f"Session closed. Synced {BOLD}{pages_copied}{RESET} pages ({total_bytes_received / 1024:.1f} KB) in {elapsed:.4f}s | Throughput: {BOLD}{speed:.2f} MB/s{RESET}", color=GREEN)
        else:
